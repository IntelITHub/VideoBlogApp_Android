...
public class TextWatcherCategorylist implements TextWatcher {
	...
	public TextWatcherCategorylist(EditText et, ListView listview,Activity a,ArrayList<String> username,ArrayList<String> category,
	...
	public void onTextChanged(CharSequence s, int start, int before, int count) {
		...
		AppSharedPrefrence.getInstance(activity).setPostOrCategory("post");
		...
		if (textlength == 0) {
			...
			myListview.setAdapter(new AllPostAdapter(activity,UsernameListCopy,PostCategoryListCopy,PostTitleListCopy,PostImageListcopy,PostLikesCopy,PostCommentsCopy,JsontempDataListCopy,PostDescriptionCopy));
		} else {
...
//			listView.setAdapter(new CustomAlertAdapterPlace(activity, serachList,CommonUtil.Iconlist()));
			myListview.setAdapter(new AllPostAdapter(activity,UsernameList,PostCategoryList,PostTitleList,PostImageList,PostLikes,PostComments,JsontempData,postDescription));
		}
	}
